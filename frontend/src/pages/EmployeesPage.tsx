...
import { Employee, formatDate } from '../lib/types';
...
export default function EmployeesPage() {
  ...
  const load = () => {
    setLoading(true);
    employeesApi.list({ search, limit: '200' })
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        setToast('Employee / labour registered');
      }
      setShowForm(false);
      load();
    } catch (err: unknown) {
      setError((err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message || 'Failed to save employee');
    }
  };
  ...
  const handleDelete = async (row: Employee) => {
    ...
    try {
      ...
      setError((err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message || 'Failed to delete employee');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold">Employees / Labours</h1>
          <p className="text-gray-500">Register workers for labour and salary wage expenses</p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          placeholder="Search name, code, phone, job title..."
          value={search}
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={row.id} className="border-t">
                ...
                <td className="p-3">
                  {row.isActive !== false ? (
                    <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">Active</span>
                  ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-semibold">{editingId ? 'Edit employee' : 'Register employee / labour'}</h2>
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              Active (available for wage expenses)
            </label>
            {error && <p className="text-sm text-rose-700">{error}</p>}
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" className="btn-secondary" onClick={() => setShowForm(false)}>Cancel</button>
              <button type="submit" className="btn-primary">{editingId ? 'Update' : 'Save employee'}</button>
            </div>
          </form>
        </div>
      )}
